<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="团购标题" prop="title" width="120" />
        <el-table-column align="left" label="商品名称" prop="goodsName" width="120" />
        <el-table-column align="left" label="剩于可团人数" prop="goodsTotal" width="120" >
          <template #default="scope">
            {{(scope.row.goodsTotal - scope.row.follow) < 1 ?"抢光了":scope.row.goodsTotal - scope.row.follow}}
          </template>
        </el-table-column>
        <el-table-column align="left" label="商品价格" prop="price" width="120" />
        <el-table-column align="left" label="团购时长" prop="time" width="120" />
...
const getTableData = async() => {
  const table = await getHelpGrouponList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHelpGrouponByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHelpGrouponFunc = async(row) => {
    const res = await findHelpGroupon({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHelpGrouponFunc = async (row) => {
    const res = await deleteHelpGroupon({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

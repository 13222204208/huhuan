<template>
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="联系人">
          <el-input v-model="searchInfo.linkman" placeholder="搜索条件" />
        </el-form-item>
        ...
        <el-form-item label="出发城市">
          <el-input v-model="searchInfo.startCity" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item label="抵达城市">
          <el-input v-model="searchInfo.arriveCity" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="物品名称" prop="goodsName" width="120" />
        <el-table-column align="left" label="物品数量" prop="goodsCount" width="120" />
        ...
        <el-table-column align="left" label="取货地址" prop="fetchAddress" width="120" />
        <el-table-column align="left" label="收货地址" prop="receiveAddress" width="220" />
        <el-table-column align="left" label="出发时间" prop="time" width="120" />
        ...
        <el-table-column align="left" label="赏金" prop="price" width="120" />
        ...
        <el-table-column align="left" label="用户编号" prop="userNum" width="120" />
        <el-table-column align="left" label="商品价值" prop="goodsPrice" width="120" />
        ...
        <el-table-column align="left" label="出发城市" prop="startCity" width="120" />
        <el-table-column align="left" label="抵达城市" prop="arriveCity" width="120" />
        <el-table-column align="left" label="航班号" prop="flightNumber" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <!-- <el-button type="text" icon="edit" size="small" class="table-button" @click="updateHelpIncidentallyFunc(scope.row)">变更</el-button> -->
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="商品价值:">
          <el-input-number v-model="formData.goodsPrice"  style="width:100%" :precision="2" clearable />
        </el-form-item>
        <el-form-item label="订单状态:">
...
import {
  ...
  deleteHelpIncidentally,
  deleteHelpIncidentallyByIds,
  updateHelpIncidentally,
  findHelpIncidentally,
  getHelpIncidentallyList
...
const getTableData = async() => {
  const table = await getHelpIncidentallyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteHelpIncidentallyFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteHelpIncidentallyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHelpIncidentallyFunc = async(row) => {
    const res = await findHelpIncidentally({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHelpIncidentallyFunc = async (row) => {
    const res = await deleteHelpIncidentally({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

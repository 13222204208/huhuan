<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="事务详情" prop="contents" width="220" />
        <el-table-column align="left" label="联系人" prop="linkman" width="120" />
        <el-table-column align="left" label="联系电话" prop="contactPhone" width="220" />
        <el-table-column align="left" label="维修地址" prop="address" width="220" />
        <el-table-column align="left" label="帮忙时长" prop="time" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <!-- <el-button type="text" icon="edit" size="small" class="table-button" @click="updateHelpWorkFunc(scope.row)">变更</el-button> -->
...
const getTableData = async() => {
  const table = await getHelpWorkList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHelpWorkByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHelpWorkFunc = async(row) => {
    const res = await findHelpWork({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHelpWorkFunc = async (row) => {
    const res = await deleteHelpWork({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="物品名称" prop="goodsName" width="220" />
        ...
        <el-table-column align="left" label="购买地址" prop="buyAddress" width="220" />
        <el-table-column align="left" label="收货地址" prop="receiveAddress" width="220" />
        <el-table-column align="left" label="送达时间" prop="time" width="120" />
        <el-table-column align="left" label="留言" prop="remark" width="120" />
      <el-table-column align="left" label="优惠券" prop="couponInfo" width="120" >
          <template #default="scope">
            {{scope.row.couponInfo.title == ""?"无":scope.row.couponInfo.title }}
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <!-- <el-button type="text" icon="edit" size="small" class="table-button" @click="updateHelpBuyFunc(scope.row)">变更</el-button> -->
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="购买地址:">
          <el-input v-model="formData.buyAddress" clearable placeholder="请输入" />
        </el-form-item>
...
const getTableData = async() => {
  console.log("搜索的信息",searchInfo.value)
  const table = await getHelpBuyList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHelpBuyByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHelpBuyFunc = async(row) => {
    const res = await findHelpBuy({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehelpBuy
...
// 删除行
const deleteHelpBuyFunc = async (row) => {
    const res = await deleteHelpBuy({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createHelpBuy(formData.value)
          break
        case 'update':
          res = await updateHelpBuy(formData.value)
          break
        default:
          res = await createHelpBuy(formData.value)
          break

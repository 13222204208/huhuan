<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="取件地址" prop="fetchAddress" width="220" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <!-- <el-button type="text" icon="edit" size="small" class="table-button" @click="updateHelpFetchFunc(scope.row)">变更</el-button> -->
...
const getTableData = async() => {
  const table = await getHelpFetchList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const setOptions = async () =>{
    ...
    console.log("字典", orderStateOptions.value )
}
...
const onDelete = async() => {
      ...
      const res = await deleteHelpFetchByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHelpFetchFunc = async(row) => {
    const res = await findHelpFetch({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rehelpFetch
...
// 删除行
const deleteHelpFetchFunc = async (row) => {
    const res = await deleteHelpFetch({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createHelpFetch(formData.value)
          break
        case 'update':
          res = await updateHelpFetch(formData.value)
          break
        default:
          res = await createHelpFetch(formData.value)
          break

...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findSystemMessage({ ID: route.query.id })
      if (res.code === 0) {
        formData.value = res.data.resystemMessage
        type.value = 'update'
      }
    } else {
      type.value = 'create'
...
const save = async() => {
      ...
      switch (type.value) {
        case 'create':
          res = await createSystemMessage(formData.value)
          break
        case 'update':
          res = await updateSystemMessage(formData.value)
          break
        default:
          res = await createSystemMessage(formData.value)
          break

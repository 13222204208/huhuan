  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="用户" prop="uid" width="120" >
          <template #default="scope">{{ scope.row.UserInfo.nickname }}</template>
        </el-table-column>
        <el-table-column align="left" label="类型" prop="type" width="120" >
          <template #default="scope">
            <view v-if="scope.row.type == 1">充值</view>
            <view v-if="scope.row.type == 2">提现</view>
            <view v-if="scope.row.type == 3">余额支付</view>
            <view v-if="scope.row.type == 4">收益</view>

          </template>
        </el-table-column>
        <el-table-column align="left" label="订单号" prop="orderNum" width="220" />
        <el-table-column align="left" label="金额" prop="money" width="120" />
        <el-table-column align="left" label="备注" prop="remark" width="320" />
        <!-- <el-table-column align="left" label="状态" prop="status" width="120" /> -->
        
        <!-- <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateMoneyRecordFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="金额:">
          <el-input-number v-model="formData.money"  style="width:100%" :precision="2" clearable />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getMoneyRecordList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteMoneyRecordByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMoneyRecordFunc = async(row) => {
    const res = await findMoneyRecord({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMoneyRecordFunc = async (row) => {
    const res = await deleteMoneyRecord({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'

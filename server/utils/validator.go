	...
	for i := 0; i < num; i++ {
		...
		if len(roleMap[tagVal.Name]) > 0 {
			for _, v := range roleMap[tagVal.Name] {
				...
				case strings.Split(v, "=")[0] == "mobile":
					//判断是否手机号验证
					//等于10代表是加拿大手机号
					if len(val.String()) == 10 {
						//result, _ := regexp.MatchString("^([2-9]\\\\d{2}[2-9](?!11)\\\\d{6})$", val.String())
						//if !result {
						//	return errors.New("加拿大手机号格式验证错误")
						//}

						//if !regexpMatch(`^([2-9]\\d{2}[2-9](?!11)\\d{6}$`, val.String()) {
						//	return errors.New("加拿大手机号格式验证错误")
						//}
					} else {
						...
						if !regexpMatch("^(1[3|4|5|8][0-9]\\d{4,8})$", val.String()) {
							return errors.New("中国手机号格式验证错误")
						}
					}
